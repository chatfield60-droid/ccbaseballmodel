...
function hasDailyPregameCapture(slateDate, fetchedAt, hasUsableQuotes) {
  return Boolean(
    slateDate
    && hasUsableQuotes
    && easternDateForTimestamp(fetchedAt) === String(slateDate),
  );
}
...
const APP_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .market-performance-card-head { display: flex; align-items: flex-start; justify-content: space-between; flex-wrap: wrap; gap: 6px 7px; }
  .market-performance-card-head h4 { flex: 1 1 120px; margin: 0; min-width: 0; color: var(--text-primary); font-size: 14px; font-weight: 750; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  .market-performance-count { flex: 0 1 auto; max-width: 100%; overflow: hidden; padding: 3px 6px; text-overflow: ellipsis; background: var(--surface-muted); color: var(--text-secondary); }
  ...
  .market-performance-financial { display: flex; align-items: flex-end; justify-content: space-between; flex-wrap: wrap; gap: 6px 8px; min-width: 0; padding-top: 1px; }
  ...
  @media (max-width: 1360px) {
    .results-market-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); }
  }
  ...
  @media (max-width: 767px) {
    ...
    .k-input-stats { grid-template-columns: repeat(2, minmax(0, 1fr)); }
...
function resultMarketGroups(rows) {
  ...
  return [...groups.values()]
    .map((group) => ({
      ...group,
      // The public materializer already emits every date in scheduled
      // first-pitch order. Preserve it so a late pending game cannot land
      // between earlier settled games because of alphabetical title sorting.
      bets: [...group.bets],
    }))
...
function slateEdgeGroups(edges, limit = 3) {
  ...
  return SLATE_EDGE_GROUPS.map((group) => {
    ...
    return { ...group, edges: matches.slice(0, limit) };
...
function ResultsPerformance({ rows, date, currentEdgeCounts, currentEdgeTotal }) {
  ...
  return <details className="card performance-disclosure" id="performance">
        ...
        {tabMarketGroups.map((group) => {
          ...
          return <article className={`market-performance-card ${tone}`} key={group.info.key} aria-label={`${group.info.label}: record ${record}, net ${net}, ROI ${roi}`}>
            <div className="market-performance-card-head">
              ...
              {liveEdgeCount ? <span className="market-performance-count">Today: {liveEdgeCount} posted edge{liveEdgeCount === 1 ? "" : "s"}</span> : null}
            </div>
    ...
    <details className="results-details">
      <summary>Posted edges by bet type ({bets.length})</summary>
        ...
        {marketGroups.map((group) => <article className="market-result-group" key={group.info.key}>
          <div className="market-result-head">
            <div>
              ...
              <span>{group.summary.posted} posted edge{group.summary.posted === 1 ? "" : "s"} · {group.summary.pending ? `${group.summary.pending} open` : `${group.summary.settled} settled`}</span>
            </div>
          ...
          {resultDateGroups(group.bets, date).map((dateGroup) => <section className="market-result-date" key={dateGroup.date}>
            <div className="market-result-date-head"><strong>{dateGroup.date}</strong><span>{dateGroup.bets.length} posted edge{dateGroup.bets.length === 1 ? "" : "s"}</span></div>
...
function buildGameDisplay(game, odds = blankOdds(), kMode = "base", kLineOverrides = {}) {
  ...
  const postedBatterPropRows = limitRowsPerTeam(
    batterPropRows
      .filter((angle) => isActionTone(angle.designation?.tone))
      .sort((a, b) => (b.designation?.edgeScore ?? -999) - (a.designation?.edgeScore ?? -999)),
    3,
  );
  ...
  const pricedBatterRows = postedBatterPropRows.map((angle) => ({
    kind: "batterProp",
    key: angle.key,
    team: angle.team,
    market: angle.market,
...
function CustomerBoard() {
  ...
  const fullSlateMarketEdges = useMemo(
    () => fullSlatePricedEdges.filter((edge) => edge.category === "Market"),
    [fullSlatePricedEdges],
  );
